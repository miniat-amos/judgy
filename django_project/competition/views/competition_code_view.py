...
def competition_code_view(request, code):
    ...
    for problem in problems:
        ...
        if problem.score_preference:  # Higher Score is Better
            problem.competition_best_score = str(
                competition_submissions.aggregate(Max('score'))['score__max'] or ""
            )
            problem.team_best_score = str(
                team_submissions.aggregate(Max('score'))['score__max'] or ""
            )
            problem.user_best_score = str(
                user_submissions.aggregate(Max('score'))['score__max'] or ""
            )
        else:  # Lower Score is Better
            problem.competition_best_score = str(
                competition_submissions.aggregate(Min('score'))['score__min'] or ""
            )
            problem.team_best_score = str(
                team_submissions.aggregate(Min('score'))['score__min'] or ""
            )
            problem.user_best_score = str(
                user_submissions.aggregate(Min('score'))['score__min'] or ""
            )
